...
public:
	...
	// Обработчик получения ввода комбо от ComboComponent
	UFUNCTION()
	void HandleComboInputReceived(int32 NewComboCount);

	// Обработчик истечения окна комбо от ComboComponent
	UFUNCTION()
	void HandleComboWindowExpired(int32 ComboCount);
